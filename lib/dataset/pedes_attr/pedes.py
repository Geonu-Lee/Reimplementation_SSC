...
class PedesAttr(data.Dataset):
    ...
    def __init__(self, cfg, split, transform=None, target_transform=None, idx=None):
        ...
        assert cfg.DATASET.NAME in ['PETA', 'PA100k', 'RAP', 'RAP2', 'RAP_align', 'PA100K_align', "PETA_misalign"], \
            f'dataset name {cfg.DATASET.NAME} is not exist'
        zero_shot = cfg.DATASET.ZERO_SHOT
        # data_path = get_pkl_rootpath(cfg.DATASET.NAME, cfg.DATASET.ZERO_SHOT)
        root = os.path.join("./data", f"{cfg.DATASET.NAME}")
        if zero_shot:
        ...
        self.samples_num = np.zeros(len(self.label[0]))
        for l in self.label:
            self.samples_num += l
